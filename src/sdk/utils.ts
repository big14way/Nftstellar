...
export function getServer(rpcUrl: string): SorobanClient.Server {
  return new SorobanClient.Server(rpcUrl, { allowHttp: true });
}

export function getContract(
  contractId: string,
  server: SorobanClient.Server
): SorobanClient.Contract {
  return new SorobanClient.Contract(contractId);
}
...
export async function getContractValue<T>(
  ...
  try {
    ...
    return SorobanClient.scValToNative(result) as T;
  } catch (error) {
